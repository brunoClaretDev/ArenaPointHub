...
public class CategoryService {
    ...
    public CategoryResponseDTO createCategory(CategoryRequestDTO dto) {
        ...
        Tournament tournament = tournamentRepository.findById(dto.getTournamentId())
                .orElseThrow(() -> new BusinessException(
                        "Torneio não encontrado com o ID: " + dto.getTournamentId()));
    ...
    public void generatePlayoffs(Long categoryId) {
        ...
        if (groups == null || groups.isEmpty()) {
            throw new BusinessException(
                    "A categoria não possui grupos cadastrados.");
        }
        ...
        for (Group group : groups) {
            ...
            List<Match> groupMatches =
                    matchRepository.findByGroupId(group.getId());
            ...
            for (Match m : groupMatches) {
                ...
                if (m.getPlayer1() != null) {
                    playerWins.putIfAbsent(m.getPlayer1(), 0);
                }

                if (m.getPlayer2() != null) {
                    playerWins.putIfAbsent(m.getPlayer2(), 0);
                }
            }
            ...
            for (Match m : groupMatches) {
                ...
                if (m.getStatus() == MatchStatus.FINISHED) {
                    ...
                    if (m.getScorePlayer1() > m.getScorePlayer2()) {
                        ...
                        playerWins.put(
                                m.getPlayer2(),
                                playerWins.get(m.getPlayer2()) + 1
                        );
                    }
                }
            }

            List<Player> sortedGroupPlayers = playerWins.entrySet()
                    .stream()
                    .sorted(
                            Map.Entry
                                    .<Player, Integer>comparingByValue()
                                    .reversed()
                    )
            ...
            if (sortedGroupPlayers.size() < 2) {
                throw new BusinessException(
                        "O grupo precisa ter pelo menos 2 jogadores computados.");
            }
        ...
        if (qualifiedPlayers.size() < 8) {
            throw new BusinessException(
                    "São necessários pelo menos 8 jogadores classificados para gerar os playoffs.");
        }
    ...
    public void validatePlayerAgeForCategory(
            Player player,
            Category category) {
        ...
        if (category.getType() == CategoryType.AGE) {
            ...
            if (player.getBirthDate() == null) {
                throw new BusinessException(
                        "A data de nascimento do jogador é obrigatória para categorias restritas por idade.");
            }
            ...
            if (category.getMaxAge() != null
                    && age > category.getMaxAge()) {

                throw new BusinessException(
                        "O jogador tem " + age
                                + " anos e excede a idade máxima de "
                                + category.getMaxAge()
            ...
            if (category.getMinAge() != null
                    && age < category.getMinAge()) {

                throw new BusinessException(
                        "O jogador tem " + age
                                + " anos e está abaixo da idade mínima de "
                                + category.getMinAge()
                                + " anos para a categoria "
                                + category.getName());
            }
    ...
    @Transactional(readOnly = true)
    public List<Player> getPlayersByCategory(Long categoryId) {

        Category category = categoryRepository.findById(categoryId)
                .orElseThrow(() -> new BusinessException(
                        "Categoria não encontrada com o ID: " + categoryId));

        return category.getPlayers();
    }
    
    @Transactional
    public void removePlayerFromCategory(Long categoryId, Long playerId) {
        ...
        Player player = playerRepository.findById(playerId)
                .orElseThrow(() -> new BusinessException(
                        "Jogador não encontrado com o ID: " + playerId));

        boolean registered = category.getPlayers()
                .stream()
                .anyMatch(p -> p.getId().equals(player.getId()));

        if (!registered) {
            throw new BusinessException(
                    "O jogador não está inscrito na categoria.");
        }

        category.getPlayers().removeIf(
                p -> p.getId().equals(player.getId())
        );
    ...
    @Transactional
    public void addPlayerToCategory(
            Long categoryId,
            Long playerId) {
        ...
        Player player = playerRepository.findById(playerId)
                .orElseThrow(() -> new BusinessException(
                        "Jogador não encontrado com ID: " + playerId));

        // Valida se o jogador cumpre os requisitos da categoria
        validatePlayerAgeForCategory(player, category);

        // Verifica se o jogador já está inscrito
        boolean alreadyRegistered = category.getPlayers()
                .stream()
                .anyMatch(p -> p.getId().equals(player.getId()));

        if (alreadyRegistered) {
            throw new BusinessException(
                    "O jogador já está inscrito na categoria.");
        }
    ...
    @Transactional(readOnly = true)
    public List<Player> drawPlayersByCategory(Long categoryId) {
        ...
        if (players.isEmpty()) {
            throw new BusinessException(
                    "Não existem jogadores inscritos na categoria.");
        }
